...
async function addTransaction(userId: string, transactionData: Omit<TransactionData, 'id' | 'userId' | 'createdAt'>): Promise<string> {
...
// Deposit Function
export const createDeposit = functions.https.onCall(async (data, context) => {
  try {
...
export const createTrade = functions.https.onCall(async (data, context) => {
  try {
    ...
    // Create transaction record
    await addTransaction(userId, {
      type: 'trade_loss', // Initially recorded as loss
      amount: -amount,
      balanceBefore: userData.balance,
      balanceAfter: newBalance,
    ...
    switch (dataType) {
      case 'transactions':
        const transactionsSnapshot = await db.collection(COLLECTIONS.TRANSACTIONS)
          ...
          .limit(50)
          .get();
        result = transactionsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        break;
      ...
      case 'deposits':
        const depositsSnapshot = await db.collection(COLLECTIONS.DEPOSITS)
          ...
          .orderBy('createdAt', 'desc')
          .get();
        result = depositsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        break;
      ...
      case 'withdrawals':
        const withdrawalsSnapshot = await db.collection(COLLECTIONS.WITHDRAWALS)
          ...
          .orderBy('createdAt', 'desc')
          .get();
        result = withdrawalsSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        break;
      ...
      case 'trades':
        const tradesSnapshot = await db.collection(COLLECTIONS.TRADES)
          ...
          .orderBy('createdAt', 'desc')
          .get();
        result = tradesSnapshot.docs.map(doc => ({ id: doc.id, ...doc.data() }));
        break;
